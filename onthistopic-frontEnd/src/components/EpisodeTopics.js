...
const EpisodeTopics = ({ episode, topics, startLoadingTopics }) => {
  ...
  //   console.log(topics.topics.topics.isLoading);
  ...
  function printTopics() {
    if (topics.topics.topics !== undefined) {
      if (topics.topics.topics.isLoading) return <div></div>;
      else
        return topics.topics.topics.map((topic) => (
          <div
            style={{
              margin: "0.5%",
